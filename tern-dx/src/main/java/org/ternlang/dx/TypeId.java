...
public final class TypeId<T> {
    ...
    TypeId(org.ternlang.dx.rop.type.Type ropType) {
        this(ropType.getDescriptor(), ropType);
    }

    TypeId(String name, org.ternlang.dx.rop.type.Type ropType) {
    ...
    public static <T> TypeId<T> get(String name) {
        return new TypeId<T>(name, org.ternlang.dx.rop.type.Type.internReturnType(name));
    }
